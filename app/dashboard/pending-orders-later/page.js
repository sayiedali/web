...
const ORDER_LATER_ENDPOINTS = [
  "/api/v1/order/later",
  "/api/v1/orders/later",
  "/api/v1/order/alllaterorder",
];
...
const page = () => {
  ...
  useEffect(() => {
    const loadLaterOrders = async () => {
      if (!branch) return;
      const endpoints = [
        `/api/v1/dashboard/later-pending-order/${branch}`,
        ...ORDER_LATER_ENDPOINTS,
      ];
      let lastError;
      ...
      setPendingOrderData([]);
      toast.error(parseApiError(lastError, "Unable to load scheduled orders."));
    };

    loadLaterOrders();
    const refreshTimer = setInterval(loadLaterOrders, 30000);
  ...
  useEffect(() => {
    ...
    const handleIncomingLaterOrder = (data) => {
    ...
    socket.on("renderOrderLater", handleIncomingLaterOrder);
    socket.on("orderCreatedLater", handleIncomingLaterOrder);
    ...
    return () => {
      ...
      socket.off("renderOrderLater", handleIncomingLaterOrder);
      socket.off("orderCreatedLater", handleIncomingLaterOrder);
  ...
  const scheduledOrders = pendingOrderData.filter(
    (item) =>
      matchesOrderType(item, "later") &&
      matchesOrderCondition(item, "pending") &&
      matchesBranch(item, branch),
  );

  const filteredScheduledOrders = scheduledOrders.filter((item) => {
    ...
    <div className="pt-[80px] p-[10px] w-full bg-slate-50 min-h-screen">
      <div className="pendingOrder-head">
        ...
        <p className="text-center text-slate-500 mt-2">
          Upcoming orders queue separated from live operations
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-8">
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-4">
          <p className="text-sm text-slate-500">Pickup Scheduled</p>
          <p className="text-2xl font-bold text-slate-800">
            {scheduledOrders.filter((item) => item.service === "pickup").length}
          </p>
        </div>
        <div className="bg-white rounded-xl border border-slate-200 p-4">
          <p className="text-sm text-slate-500">Delivery Scheduled</p>
          <p className="text-2xl font-bold text-slate-800">
            {scheduledOrders.filter((item) => item.service !== "pickup").length}
          </p>
        </div>
      ...
      <div className="w-full mt-[30px] pb-[20px] flex flex-col sm:flex-row gap-3 bg-white border border-slate-200 rounded-xl p-4">
