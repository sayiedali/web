...
const FooterLogoForm = () => {
  ...
  const handleSubmit = () => {
    ...
    let config = {
      ...
      url: `${apiUrl}/api/v1/logo/footer-logo/create`,
      headers: {
    ...
    axios
      .request(config)
      .then((res) => {
        if (res.data.message === "Footer Logo Successfully Created!") {
          setShowLoader(false);
      ...
      .catch((error) => {
        ...
        toast.error("Error adding footer logo. Please try again.");
      });
  };
  ...
  let handleDelete = (_id) => {
    ...
    axios
      .post(
        `${apiUrl}/api/v1/logo/footer-logo/delete`,
        {
          id: _id,
        },
  ...
  useEffect(() => {
    axios
      .get(`${apiUrl}/api/v1/logo/footer-logo/all`, {
        auth: {
          username: "user",
          password: getToken,
        },
  ...
  let handleUpdate = () => {
    ...
    let config = {
      method: "post",
      maxBodyLength: Infinity,
      url: `${apiUrl}/api/v1/logo/footer-logo/update`,
      headers: {
        "Content-Type": "application/json",
      },
      auth: {
      ...
      data: {
        id: editID,
        updatedFooterLogo: { ...logoData, image: imageBase64Data },
      },
    };
    ...
    axios
      .request(config)
      .then((res) => {
        if (res.data.message === "Footer Logo Successfully Updated!") {
          setEdit(false);
          setShowLoader(false);
          setEditID("");
          toast.success(res.data.message);
      ...
      .catch((error) => {
        ...
        toast.error("Error updating footer logo. Please try again.");
      });
  };
